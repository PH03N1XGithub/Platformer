...
namespace
{
	...
	inline uint64 EncodeBodyPair(uint32 a, uint32 b)
	{
		const uint32 first = (a < b) ? a : b;
		const uint32 second = (a < b) ? b : a;
		return (static_cast<uint64>(first) << 32u) | static_cast<uint64>(second);
	}

	inline uint32 DecodeBodyPairFirst(uint64 pair)
	{
		return static_cast<uint32>(pair >> 32u);
	}

	inline uint32 DecodeBodyPairSecond(uint64 pair)
	{
		return static_cast<uint32>(pair & 0xFFFFFFFFull);
	}
...
{
	struct OverlapContactListener final : public JPH::ContactListener
	{
		explicit OverlapContactListener(Impl& owner)
			: Owner(owner)
		{
		}
	...
	private:
		...
		{
			const uint32 handle1 = body1.GetID().GetIndexAndSequenceNumber();
			const uint32 handle2 = body2.GetID().GetIndexAndSequenceNumber();
			if (handle1 == 0u || handle2 == 0u || handle1 == handle2)
				return;

			const uint64 pair = EncodeBodyPair(handle1, handle2);
			std::lock_guard<std::mutex> lock(Owner.OverlapPairsMutex);
			Owner.CurrentOverlapPairs.insert(pair);
		}
	...
	OverlapContactListener ContactListener;
	JPH::TempAllocatorImpl* TempAllocator = nullptr;
	JPH::JobSystemThreadPool* JobSystem = nullptr;

	std::unordered_map<uint32, PrimitiveComponent*> BodyToPrimitive;
	std::unordered_set<uint64> CurrentOverlapPairs;
	std::unordered_set<uint64> PreviousOverlapPairs;
	std::mutex OverlapPairsMutex;
...
namespace
{
	void DispatchOverlapBegin(PrimitiveComponent* a, PrimitiveComponent* b)
	{
		if (!a || !b)
			return;
		...
		Actor* ownerA = a->GetOwner();
		Actor* ownerB = b->GetOwner();
		if (!ownerA || !ownerB || ownerA == ownerB)
			return;

		if (a->bGenerateOverlapEvents)
		{
			a->OnComponentBeginOverlap(b);
			ownerA->OnActorBeginOverlap(ownerB, a, b);
		}
		if (b->bGenerateOverlapEvents)
		{
			b->OnComponentBeginOverlap(a);
			ownerB->OnActorBeginOverlap(ownerA, b, a);
		}
	}

	void DispatchOverlapStay(PrimitiveComponent* a, PrimitiveComponent* b)
	{
		if (!a || !b)
			return;
		...
		if (a->bGenerateOverlapEvents)
		{
			a->OnComponentOverlapStay(b);
			ownerA->OnActorOverlapStay(ownerB, a, b);
		}
		if (b->bGenerateOverlapEvents)
		{
			b->OnComponentOverlapStay(a);
			ownerB->OnActorOverlapStay(ownerA, b, a);
		}
	}

	void DispatchOverlapEnd(PrimitiveComponent* a, PrimitiveComponent* b)
	{
		if (!a || !b)
			return;
		...
		if (a->bGenerateOverlapEvents)
		{
			a->OnComponentEndOverlap(b);
			ownerA->OnActorEndOverlap(ownerB, a, b);
		}
		if (b->bGenerateOverlapEvents)
		{
			b->OnComponentEndOverlap(a);
			ownerB->OnActorEndOverlap(ownerA, b, a);
		}
	}
}
...
void PhysicsSystem::Shutdown()
{
	if (m_Impl != nullptr)
	{
		...
		m_Impl->BodyToPrimitive.clear();
		m_Impl->CurrentOverlapPairs.clear();
		m_Impl->PreviousOverlapPairs.clear();
...
void PhysicsSystem::DestroyBodyForComponent(PrimitiveComponent& primitive)
{
	...
	const uint32 bodyHandle = primitive.GetBodyHandle();
	...
	{
		std::lock_guard<std::mutex> lock(m_Impl->OverlapPairsMutex);
		auto endOverlapsForSet = [&](std::unordered_set<uint64>& overlaps)
		{
			for (auto it = overlaps.begin(); it != overlaps.end();)
			{
				const uint64 pair = *it;
				const uint32 a = DecodeBodyPairFirst(pair);
				const uint32 b = DecodeBodyPairSecond(pair);
				if (a != bodyHandle && b != bodyHandle)
				{
					++it;
					continue;
				}

				const uint32 otherHandle = (a == bodyHandle) ? b : a;
				PrimitiveComponent* other = nullptr;
				auto otherIt = m_Impl->BodyToPrimitive.find(otherHandle);
				if (otherIt != m_Impl->BodyToPrimitive.end())
					other = otherIt->second;

				DispatchOverlapEnd(&primitive, other);
				it = overlaps.erase(it);
			}
		};

		endOverlapsForSet(m_Impl->CurrentOverlapPairs);
		endOverlapsForSet(m_Impl->PreviousOverlapPairs);
		m_Impl->BodyToPrimitive.erase(bodyHandle);
	}
...
void PhysicsSystem::Step(Scene& scene, Float dt)
{
	...
	for (auto entity : view)
	{
		...
		const uint32 bodyHandle = body->GetID().GetIndexAndSequenceNumber();
		primitive->SetBodyHandle(bodyHandle);
		m_Impl->BodyToPrimitive[bodyHandle] = primitive;
	}
	...
	if (m_LastSubstepCount > 0)
	{
		std::unordered_set<uint64> currentPairs;
		{
			std::lock_guard<std::mutex> lock(m_Impl->OverlapPairsMutex);
			currentPairs = m_Impl->CurrentOverlapPairs;
		}

		auto resolvePair = [&](uint64 pair, PrimitiveComponent*& outA, PrimitiveComponent*& outB)
		{
			outA = nullptr;
			outB = nullptr;
			const uint32 aHandle = DecodeBodyPairFirst(pair);
			const uint32 bHandle = DecodeBodyPairSecond(pair);
			auto aIt = m_Impl->BodyToPrimitive.find(aHandle);
			auto bIt = m_Impl->BodyToPrimitive.find(bHandle);
			if (aIt != m_Impl->BodyToPrimitive.end())
				outA = aIt->second;
			if (bIt != m_Impl->BodyToPrimitive.end())
				outB = bIt->second;
		};

		for (const uint64 pair : currentPairs)
		{
			PrimitiveComponent* a = nullptr;
			PrimitiveComponent* b = nullptr;
			resolvePair(pair, a, b);
			if (!a || !b)
				continue;

			if (m_Impl->PreviousOverlapPairs.find(pair) == m_Impl->PreviousOverlapPairs.end())
				DispatchOverlapBegin(a, b);
			else
				DispatchOverlapStay(a, b);
		}

		for (const uint64 pair : m_Impl->PreviousOverlapPairs)
		{
			...
			PrimitiveComponent* b = nullptr;
			resolvePair(pair, a, b);
			DispatchOverlapEnd(a, b);
		}

		m_Impl->PreviousOverlapPairs = std::move(currentPairs);
	}
