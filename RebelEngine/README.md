...
**Current milestone:** animation runtime evaluation & locomotion pipeline  
**Next milestone:** Character Controller & Movement System
...
The engine already has a skeletal rendering foundation.  
Next is runtime animation pose evaluation, followed by locomotion systems that integrate cleanly with physics and camera behavior.
...
This flow guides upcoming runtime implementation.
...
- **Skeletal rendering base:** skeletal assets render, and runtime animation evaluation is the next milestone.
...
- Skin palette uses an identity pose while runtime animation evaluation is in progress.
...
- **Character/locomotion**:
  - Dedicated character movement component/controller is planned next.
- **Skeletal/animation**:
  - Skeletal meshes and skeleton assets load and render.
  - Skeleton debug drawing is active.
  - Animation module exists and runtime behavior is planned next (`RebelEngine/src/Animation/AnimationModule.cpp`).

### Planned (next phase)

Planned high-level flow for scalable locomotion/animation integration:

```text
Intent -> MovementRequest -> MotionDelta -> AnimGraphOutput -> CameraRigOutput
```

This roadmap outlines upcoming runtime behavior.
...
1. Implement runtime animation evaluation in `AnimationModule` (replace identity bone palettes with evaluated pose data).
2. Introduce explicit movement intent/motion components and systems aligned with existing tick phases.
3. Add a character controller layer (physics-backed) with deterministic movement update stage before render.
4. Expand camera rig stack (follow/orbit/aim behaviors) on top of current camera separation.
5. Strengthen asset import tooling for animation clips and graph/state definitions.
