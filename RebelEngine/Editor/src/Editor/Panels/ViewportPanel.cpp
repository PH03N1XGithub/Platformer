...
String BuildLevelTimerLabel(const float elapsedSeconds)
{
    const float clampedSeconds = glm::max(0.0f, elapsedSeconds);
    const int totalMilliseconds = static_cast<int>(clampedSeconds * 1000.0f);
    ...
    char buffer[64];
    std::snprintf(buffer, sizeof(buffer), "%s %02d:%02d.%03d", ICON_FA_STOPWATCH, minutes, seconds, milliseconds);
...
void ViewportPanel::DrawViewportStatus(const ImVec2& imagePos, const ImVec2& imageMax, const ImVec2& viewportSize, int fps)
{
    ...
    if (GEngine && GEngine->IsPlaying() && LevelStarterActor::IsRuntimeTimerVisible())
    {
        const String timerLabel = BuildLevelTimerLabel(LevelStarterActor::GetRuntimeElapsedSeconds());
        DrawStatusChip(
            ImVec2(imagePos.x + 12.0f, imageMax.y - 64.0f),
            timerLabel,
            IM_COL32(14, 32, 22, 228),
            IM_COL32(196, 244, 206, 255));
    }
}
